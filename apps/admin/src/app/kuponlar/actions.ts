...
function validate(input: CouponInput) {
  if (!input.code || input.code.trim().length < 2) {
    throw new Error('Kupon kodu en az 2 karakter olmalı.')
  }
  ...
  if (input.starts_at && input.expires_at) {
    const s = new Date(input.starts_at).getTime()
    const e = new Date(input.expires_at).getTime()
...
function toRow(input: CouponInput, userId: string | null) {
  return {
...
export async function createCoupon(input: CouponInput) {
  validate(input)
  const { supabase, userId } = await requireAdmin('/kuponlar')
  const { error } = await supabase.from('coupons').insert(toRow(input, userId))
  if (error) {
