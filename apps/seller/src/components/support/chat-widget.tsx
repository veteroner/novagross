...
const GREETING: Msg = {
  role: 'assistant',
  content:
    'Merhaba! Novagross satıcı destek asistanıyım 👋 Komisyon, ödeme, ürün onayı, kargo veya başka bir konuda nasıl yardımcı olabilirim? Talebiniz yöneticiye iletilir.',
}

export function SellerSupportWidget() {
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch('/api/support/chat', {
        ...
        body: JSON.stringify({ messages: next.filter((m) => m.content !== GREETING.content), ticketId }),
      })
    ...
    <>
      {!open && (
        ...
        >
          ...
          <span className="hidden sm:inline text-sm font-medium">Destek</span>
        </button>
      )}
      ...
      {open && (
        <div className="fixed bottom-5 right-5 z-[60] w-[92vw] max-w-sm h-[70vh] max-h-[560px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border">
          <div className="flex items-center justify-between px-4 py-3 text-white" style={{ backgroundColor: '#16A34A' }}>
            <div className="flex items-center gap-2">
              <MessageCircle className="h-5 w-5" />
              <div>
                <p className="text-sm font-semibold leading-tight">Satıcı Destek</p>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-2 bg-gray-50">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap ${
                  m.role === 'user' ? 'bg-green-600 text-white rounded-br-sm' : 'bg-white border rounded-bl-sm'
                }`}>
                  {m.content}
                </div>
              </div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-white border rounded-2xl rounded-bl-sm px-3 py-2"><Loader2 className="h-4 w-4 animate-spin text-gray-400" /></div>
              </div>
            )}
          </div>
          ...
          <div className="p-2 border-t flex items-end gap-2">
            ...
            />
            <button onClick={send} disabled={loading || !input.trim()} className="rounded-lg p-2 text-white disabled:opacity-50" style={{ backgroundColor: '#16A34A' }} aria-label="Gönder">
