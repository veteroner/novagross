...
export default async function OrderDetailPage({
  ...
  // Already-opened claims for these order items
  const itemIds = (items ?? []).map((i: any) => i.id)
  const { data: existingClaims } =
    itemIds.length > 0
      ? await (supabase as any)
          .from('customer_claims')
          .select('id, status, claim_type, order_item_id')
          .in('order_item_id', itemIds)
      : { data: [] }
  const claimByItem = new Map<string, any>()
  for (const c of existingClaims ?? []) {
    if (!claimByItem.has(c.order_item_id)) claimByItem.set(c.order_item_id, c)
  }
        ...
        <CardContent>
          <div className="space-y-4">
            {(items ?? []).map((item: any) => {
              ...
              const existingClaim = claimByItem.get(item.id) ?? null
              return (
